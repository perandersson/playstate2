...
: mParent(NULL), mChildren(offsetof(SpatialSceneNode, SpatialSceneNodeLink)), mBoundingBox(boundingBox), mDepth(0), mMaxDepth(maxDepth), mAlive(false)
{
...
: mParent(parent), mChildren(offsetof(SpatialSceneNode, SpatialSceneNodeLink)), mBoundingBox(boundingBox),
...
bool QuadTree::Add(SpatialSceneNode* node)
{
	// Ignore nodes not completely inside this quadtree node's bounding box.
	CollisionResult::Enum result = mBoundingBox.IsColliding(node->GetBoundingBox());
	if (result != CollisionResult::CONTAINED) {
		if (IsRootNode()) {
			AddToThisNode(node);
			return true;
		}
...
void QuadTree::Remove(SpatialSceneNode* node)
{
	// Remove the node
	mChildren.Remove(node);
	node->SetSpatialTree(nullptr);
...
void QuadTree::Absorb(LinkedList<SpatialSceneNode>& children)
{
	SpatialSceneNode* node = children.First();
	while (node != NULL) {
		SpatialSceneNode* next = node->SpatialSceneNodeLink.Tail;
		mChildren.AddLast(node);
		node->SetSpatialTree(this);
		node = next;
	}
}
...
void QuadTree::AddToRoot(SpatialSceneNode* node)
{
	// Is the node still inside this QuadTreeNode?
	CollisionResult::Enum result = mBoundingBox.IsColliding(node->GetBoundingBox());
	if (result == CollisionResult::CONTAINED || IsRootNode()) {
		AddToThisNode(node);
		return;
	}
...
void QuadTree::AddToThisNode(SpatialSceneNode* node)
{
	mChildren.AddLast(node);
	node->SetSpatialTree(this);
	mAlive = true;
...
void QuadTree::Invalidate(SpatialSceneNode* node)
{
