...
namespace core
{
	...
	public:
		...
		inline bool IsLeafNode() const {
			return mCorners[0] == nullptr || AtMaxDepth();
		}
	...
	private:
		...
		void Absorb(LinkedList<SpatialSceneNode>& children);
		...
		void AddToRoot(SpatialSceneNode* node);

		//
		// Add the supplied node to this node. 
		void AddToThisNode(SpatialSceneNode* node);
	...
	private:
		QuadTree* mParent;
		QuadTree* mCorners[4];
		LinkedList<SpatialSceneNode> mChildren;
		AABB mBoundingBox;
		uint32 mDepth;
		uint32 mMaxDepth;
		bool mAlive;
