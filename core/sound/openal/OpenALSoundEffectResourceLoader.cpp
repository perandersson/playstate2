#include "../../Memory.h"
#include "OpenALSoundEffectResourceLoader.h"
...
ResourceObject* OpenALSoundEffectResourceLoader::Load(const IFile* file)
{
	const std::string absolutePath = file->GetAbsolutePath();

	WAVHeader header;
	if(file->ReadBytes((byte*)&header, sizeof(WAVHeader)) != sizeof(WAVHeader))
		THROW_EXCEPTION(LoadResourceException, "Could not load resource: '%s'. Header was invalid", absolutePath.c_str());
	...
	std::unique_ptr<byte[]> data(new byte[header.Subchunk2Size]);
	if (file->ReadBytes(data.get(), sizeof(WAVHeader), header.Subchunk2Size) != header.Subchunk2Size)
		THROW_EXCEPTION(LoadResourceException, "Could not load resource: '%s'. Data size was invalid", absolutePath.c_str());
	...
	if(format != SoundFormat::MONO8 && format != SoundFormat::MONO16) {
		Logger::Warn("The loaded sound effect: '%s' is not a MONO sound. 3D positioning will not work",
			file->GetAbsolutePath().c_str());
	}

	const float32 duration = header.SampleRate / (header.BitsPerSample / 8.0f) / (header.SampleRate * 1.0f);
	if(duration > 5.0f) {
		Logger::Warn("The loaded sound effect: '%s' is longer than 5 seconds.",
			file->GetAbsolutePath().c_str());
	}
	...
	if(error != AL_NO_ERROR)
		THROW_EXCEPTION(LoadResourceException, "Could not load sound buffer with data for sound: '%s'", file->GetAbsolutePath().c_str());
...
ResourceObject* OpenALSoundEffectResourceLoader::GetDefaultResource()
{
	if(mDefaultResource == NULL) {
		std::shared_ptr<IFile> file = FileSystem::OpenFile("/engine/defaults/sound_effect.wav");
		assert(file->Exists() && "The default resource does not exist");
		mDefaultResource = Load(file.get());
	}
